...
const FED_PRESS_RELEASES_URL = 'https://www.federalreserve.gov/feeds/press_all.xml';
...
function extractItems(xml) {
  ...
  return items.map((item, index) => ({
    ...
    guid: matchTag(item, 'guid') || `fed-${index + 1}`
  }));
}

function normalizeItem(item, collectedAt, index) {
  return {
    id: `fed-${new Date(collectedAt).toISOString().slice(0, 10)}-${String(index + 1).padStart(3, '0')}`,
...
async function main() {
  ...
  const rawXml = await fetchText(FED_PRESS_RELEASES_URL);
  const rawName = `${isoSafe(new Date())}-fed-press-releases.xml`;
  ...
  if (normalized.length > 0) {
    const outName = `${isoSafe(new Date())}-fed-normalized.json`;
    ...
    console.log(JSON.stringify({
      ...
      feed_url: FED_PRESS_RELEASES_URL,
      raw_file: path.join('data', 'raw', 'official', rawName),
      normalized_file: path.join('data', 'normalized', outName),
