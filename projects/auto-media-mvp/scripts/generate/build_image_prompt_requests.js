...
const { buildImagePromptInput, buildImagePromptRequest } = require('./lib_image_prompt');
  ...
  for (const file of listEnrichedFiles()) {
    ...
    for (const entry of entries) {
      ...
      const input = buildImagePromptInput({
        ...
        article_title: entry?.outputs?.article?.title || ''
      });
      const request = buildImagePromptRequest(input);
      const outName = `${entry.id}-image-prompt-request.json`;
