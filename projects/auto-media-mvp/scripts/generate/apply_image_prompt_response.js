...
function listResponses() {
  ...
  return fs.readdirSync(RESPONSES_DIR).filter((name) => name.endsWith('-image-prompt-response.json')).sort().map((name) => path.join(RESPONSES_DIR, name));
}
...
function main() {
  ...
  for (const responseFile of responses) {
    ...
    if (!response?.ok || !response?.item_id || !response?.image?.image_prompt) continue;
    ...
    for (const enrichedFile of enrichedFiles) {
      ...
      for (const entry of entries) {
        ...
        entry.outputs.image = {
          image_prompt: response.image.image_prompt,
          meta: {
            prompt_file: response?.meta?.prompt_file || 'prompts/image_prompt.md',
      ...
      if (touched) fs.writeFileSync(enrichedFile, JSON.stringify(entries, null, 2) + '\n', 'utf8');
    }
  }
