...
const { buildTitleInput, buildTitleRequest } = require('./lib_title');
...
function main() {
  ...
  for (const file of listEnrichedFiles()) {
    ...
    for (const entry of entries) {
      ...
      const request = buildTitleRequest(input);
      const outName = `${entry.id}-title-request.json`;
